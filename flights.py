...
for r in rows:
    ...
    if len(flt)==6:
        ...
        std=flt[1].text
        sta=flt[3].text
        flts.append([find,nbr[0:2],nbr[2:len(nbr)],("-".join(std[0:10].split(".")[::-1]))+' '+std[11:16],flt[2].text,("-".join(sta[0:10].split(".")[::-1]))+' '+sta[11:16],flt[4].text,"","",flt[5].text])
        find+=1
...
for f in flts:
    ...
    if info:
        ...
        for m in members:
            ...
            if len(c)==6:
                id=int(c[0].text)
                crew.append([find,id,c[3].text,c[4].text])
                if id!=12345:
                    psen=0
                    if c[3].text=="PU":
                        sl=c[2].text.split("/")
